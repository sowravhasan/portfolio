...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (statsShowcase) {
    statsObserver.observe(statsShowcase);
    console.log("Observing stats-showcase");
  }

  function animateStats() {
    console.log("Starting stats animation for", statNumbers.length, "elements");

    statNumbers.forEach((stat, index) => {
      // Check both data-count and data-target attributes
      const target =
      ...
      if (!target || isNaN(target)) {
        console.warn(`Stat ${index}: Invalid target value`);
        return;
      }

      let count = 0;
      const increment = target / 60; // Reduced for smoother animation
      const duration = 2000;
  ...
  // Manual trigger for debugging - remove in production
  setTimeout(() => {
    if (!hasAnimated) {
      console.log("Auto-triggering stats animation for debugging");
      animateStats();
      hasAnimated = true;
    }
  }, 3000);
  ...
  portfolioImages.forEach((img) => {
    ...
    portfolioItem.addEventListener("mouseenter", function () {
      // Create smooth scroll effect from top to bottom
      img.style.transition = "transform 4s ease-in-out";
      img.style.transform = "translateY(-20%) scale(1.02)";
  ...
  initModernPortfolioFilters();
  initModernSkills();
  optimizeIconLoading();
});

// Modern Portfolio Filter Functionality
function initModernPortfolioFilters() {
  const filterBtns = document.querySelectorAll(".filter-modern-btn");
  const portfolioItems = document.querySelectorAll(".portfolio-modern-item");
...
function initModernSkills() {
  const observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          const skillBars = entry.target.querySelectorAll(
            ".skill-modern-progress"
          );
  ...
  const skillsContainer = document.querySelector(".skills-modern-container");
  if (skillsContainer) {
...
// Optimize icon loading and reduce perceived loading time
function optimizeIconLoading() {
  ...
  function checkFontAwesome() {
    ...
    testElement.className = "fa-solid fa-heart";
    ...
    // Font Awesome is loaded if font-family contains "Font Awesome"
    return fontFamily.toLowerCase().includes("font awesome");
  }

  // Priority loading for critical icons in viewport
  const criticalIcons = document.querySelectorAll(
    ".service-icon-modern i, .highlight-icon i, .navbar i"
  );
  criticalIcons.forEach((icon) => {
    icon.style.fontDisplay = "swap";
  });

  // Check if icons are loaded with optimized timing
  let attempts = 0;
  ...
  const checkIconsLoaded = setInterval(() => {
    ...
    if (checkFontAwesome() || attempts >= maxAttempts) {
      ...
      // Smooth transition when icons are loaded
      setTimeout(() => {
        document.body.classList.remove("icons-loading");
        document.body.classList.add("icons-loaded");

        // Trigger a custom event for icon loading completion
        const event = new CustomEvent("iconsLoaded");
  ...
  // Fallback: Remove loading state after max time regardless
  setTimeout(() => {
    if (document.body.classList.contains("icons-loading")) {
      document.body.classList.remove("icons-loading");
      document.body.classList.add("icons-loaded");
      console.log("⚠️ Icon loading timeout - showing all icons anyway");
    }
  }, 5000);
}
...
// Add CSS for icon loading states
const iconLoadingStyle = document.createElement("style");
iconLoadingStyle.textContent = `
  /* Icon loading optimization */
  body.icons-loading i[class*="fa-"] {
    opacity: 0.3;
    transition: opacity 0.3s ease;
  }
  
  body.icons-loaded i[class*="fa-"] {
    opacity: 1;
  }
  
  /* Show loading placeholder for icons */
  body.icons-loading .service-icon-modern i,
  body.icons-loading .highlight-icon i,
  body.icons-loading .skill-modern-icon i {
    position: relative;
  }
  
  body.icons-loading .service-icon-modern i::before,
  body.icons-loading .highlight-icon i::before,
  body.icons-loading .skill-modern-icon i::before {
    content: '⚪';
    position: absolute;
    top: 50%;
    left: 50%;
...
`;
document.head.appendChild(iconLoadingStyle);
